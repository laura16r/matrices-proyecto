...
def medir_tiempo(algoritmo, A, B):
    """Mide el tiempo de ejecución de un algoritmo en segundos."""
...
def ejecutar_todos():
    """
    Carga los casos de prueba, ejecuta los 15 algoritmos y guarda
    los tiempos en resultados.csv.
    """
    ...
    with open(archivo, "a", newline="", encoding="utf-8") as f:
        ...
        for algo in ALGORITMOS:
            ...
            print(f"  {nombre} - Caso 1 ({algo.n1}x{algo.n1})...")
            te1 = medir_tiempo(algo, A1, B1)
            writer.writerow([nombre, "Caso1", algo.n1, round(te1, 6)])
            print(f"    TE: {te1:.4f}s")

            print(f"  {nombre} - Caso 2 ({algo.n2}x{algo.n2})...")
            te2 = medir_tiempo(algo, A2, B2)
            writer.writerow([nombre, "Caso2", algo.n2, round(te2, 6)])
            print(f"    TE: {te2:.4f}s")
